...
// Scheduled and conditional drops, watchers, claims and swap-and-send send
// through Connect's MCP connect_drop. Recipients go as given; Connect resolves
// them for the chain.
async function quidliDropOnce({ recipients, amountInWeiPerRecipient, chainId = 8453, tokenContract }, apiKey = QUIDLI_API_KEY) {
  if (!apiKey) {
    throw new Error('No Quidli API key available for this drop. Link your Connect key, or ask the bot owner to configure a host key.');
  }
  const body = JSON.parse(await mcpCallTool('connect_drop', {
    idempotencyKey: crypto.randomUUID(), chainId, tokenContract, amountInWeiPerRecipient, recipients,
  }, apiKey));
...
const tools = [
  ...
  },
  {
    ...
    description: 'Resolve a Telegram @username to its numeric Telegram ID by checking every group chat this bot has ever seen them post in. ALWAYS try this BEFORE calling connect_lookup/connect_drop with a raw Telegram username, since Telegram itself does not let Quidli resolve arbitrary usernames — only numeric IDs work reliably. If this returns a numeric ID, use { type: "telegram", id: "<id>" } for connect_lookup and connect_drop instead of username. If it returns nothing, the bot has never seen that user active anywhere, and you should tell the requester to ask them to DM this bot directly (any message, even just "hi") — after they do, this will resolve them.',
    input_schema: {
      type: 'object',
      properties: {
...
async function runTool(name, input, {
  ...
  if (mcpToolNames.has(name)) {
    ...
    try {
      ...
      if (name === 'connect_drop') {
        ...
        const explorerUrl = explorerTxUrl(input?.chainId ?? 8453, body?.transferHash);
        if (explorerUrl) _pendingExplorerUrls.push(explorerUrl);
        console.log('[drop] result:', JSON.stringify(body));
        return JSON.stringify({ ...body, explorerUrl }, null, 2);
      }
  ...
  if (name === 'resolve_telegram_username') {
    ...
    if (telegramId) {
      return JSON.stringify({ found: true, telegramId, note: `Use { type: "telegram", id: "${telegramId}" } for connect_lookup/connect_drop.` });
    }
    return JSON.stringify({
      found: false,
